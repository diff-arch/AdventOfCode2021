...
//
//  Goal:    To enable as many submarine features as possible, find the largest 
//           valid fourteen-digit model number that contains no 0 digits. 
...
int main() {
	...
	std::vector<int> range = {9, 8, 7, 6, 5, 4, 3, 2, 1};  // only digits from 1 to 9 in MONAD input
	...
	for (const auto& digits : kInputSpace) {
		...
		if (valid) {
			std::cout << "Largest Accepted Model Number: ";
			for (int& n : result)
				std::cout << n;
			std::cout << "\n";
			break;
