...
//
//  Goal:    Determine the horizontal position that the unique_positions can align to using the least fuel possible.
...
int main() {
    ...
    for (int i = 0; i < unique_positions.size(); i++) {
        for (int j = 0; j < horizontal_positions.size(); j++) {
            distances_to[unique_positions[i]].emplace(
                horizontal_positions[j], std::abs(horizontal_positions[j] - unique_positions[i])
            );
        }
    }

    int closest_distance = std::numeric_limits<int>::max();
    int closest_position, distance;

    for (int& position : crab_positions) {
        distance = 0;
        for (int& other : crab_positions)
            distance += distances_to[position][other];
