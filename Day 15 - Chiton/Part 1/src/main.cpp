...
//
//  Goal:    What is the lowest total risk of any path from 
...
int main() {
  ...
  const char* fpath = "../bin/data/chiton.txt";  // insert your path
	...
	int source = 0;
	int target = g.cells.size() - 1;
	
	std::pair<std::map<int, int>, std::map<int, int>> dk = dijkstra(g, source);
	std::vector<int> path = path_to(dk.second, source, target);
	
	int total_risk = 0;
	for (const int i : path) {
		if (i != source)
			total_risk += g.cells[i];
	}
